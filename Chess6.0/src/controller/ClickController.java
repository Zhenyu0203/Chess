            ...
            if (first == chessComponent) { // 再次点击取消选取
                ...
                if (first instanceof PawnChessComponent&&chessboard.getPreviousGraph().size()>=4){
                    ...
                    if (((PawnChessComponent) first).checkcglb(chessboard.getChessComponents(), chessComponent.getChessboardPoint())){
                        ...
                        int xf=chessComponent.getChessboardPoint().getX();
                        int yf=chessComponent.getChessboardPoint().getY();
                        chessboard.swapChessComponents(first,chessComponent);
                        ChessComponent empty=new EmptySlotComponent(new ChessboardPoint(first.getChessboardPoint().getX(),chessComponent.getChessboardPoint().getY()),
                                chessboard.calculatePoint(first.getChessboardPoint().getX(),chessComponent.getChessboardPoint().getY()), chessboard.getClickController(), chessboard.getCHESS_SIZE());
